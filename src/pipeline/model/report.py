...
def summarize_one(s: Dict[str, Any]) -> str:
    ...
    if dists.get("evidence_types_all"):
        parts.append(
            md_table("Evidence type distribution", dists["evidence_types_all"], total)
        )
    ...
    if gnn:
        ...
        parts.append(
            f"- Label balance OK: {'Yes' if balance_ok else 'No — imbalanced (>70% one label)'}\n\n"
        )
...
def plot_horizontal_bar_with_target(
    dist: Dict[str, int], targets: Dict[str, int], title: str, out_path: Path
):
...
def summarize_training(
    ...
    if et_dist and et_targets:
        ...
        plot_horizontal_bar_with_target(
            et_dist, et_targets, "Training — Evidence type vs ADR-006 targets", p
        )
    ...
    parts.append(
        ...
        f"**Total records:** {total:,}  |  `postulation_derivation`: {postulation_count}\n\n"
    )
    ...
    for et in sorted(pv.keys()):
        info = pv[et]
        parts.append(
            f"| {et} | {info['actual']:,} | {info['target']:,} | {info['delta']:+d} | {info['pct']} |\n"
        )
    parts.append("\n")
...
def main():
    ...
    ap.add_argument(
        "--training-summary",
        default=None,
        help="Optional path to training_validation.json",
    )
    args = ap.parse_args()
    ...
    if args.training_summary:
        ...
        if tv_path.exists():
            training_md = summarize_training(
                load_json(str(tv_path)), plots_dir, plot_refs
            )
