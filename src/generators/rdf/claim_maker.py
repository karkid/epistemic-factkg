...
def is_verbalizable_predicate(pred: str) -> bool:
    ps = pred.split("#")[-1].split("/")[-1].lower()
...
def verbalize(triple: Triple) -> str:
    s, p, o = triple
    subj = _pretty_objtype(s)
    pred = _short(p)
    obj = _pretty_objtype(o)
    ...
    if pred_l.endswith("isopen") or pred_l == "isopen":
        val = str(o).lower()
        return f"The {subj.lower()} is open." if val == "true" else f"The {subj.lower()} is not open."

    if pred_l.endswith("istoggled") or pred_l == "istoggled":
        val = str(o).lower()
        return f"The {subj.lower()} is switched on." if val == "true" else f"The {subj.lower()} is not switched on."

    if pred_l.endswith("isdirty") or pred_l == "isdirty":
        val = str(o).lower()
        return f"The {subj.lower()} is dirty." if val == "true" else f"The {subj.lower()} is not dirty."

    if pred_l.endswith("isbroken") or pred_l == "isbroken":
        val = str(o).lower()
        return f"The {subj.lower()} is broken." if val == "true" else f"The {subj.lower()} is not broken."

    # fallback
    # return f"{subj} has relation {pred} with {obj}."
    # fallback
    if pred_l.endswith("hasobject") or pred_l == "hasobject":
        return ""
    elif _is_type_predicate(p):
        return ""
    
    # return f"A {subj.lower()} has relation {pred.lower()} with the {obj.lower()}."
    return ""
