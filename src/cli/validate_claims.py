...
def main():
    ...
    try:
        ...
        if not ttl_path.exists():
            logger.warning(
                f"TTL file not found: {args.ttl_path} (validation will proceed without RDF context)"
            )

        logger.info("Starting claims validation...")
        ...
        if args.export_detailed_report or args.export_errors:
                        ...
                        issues_to_export.append(
                            {
                                ...
                                "claim_text": summary.claim_texts.get(
                                    result.claim_id, "N/A"
                                ),
            ...
            if issues_to_export:
                ...
                if args.export_detailed_report:
                    ...
                    logger.info(
                        f"📁 Detailed validation report exported to: {report_file}"
                    )

                if args.export_errors and any(
                    not summary.results[i].is_valid for i in range(len(summary.results))
                ):
                    error_file = output_dir / "validation_errors.jsonl"
                    error_issues = [
                        issue
                        for issue in issues_to_export
                        if issue["issue_severity"] == "error"
                    ]
        ...
        if invalid_count == 0:
            ...
            logger.error(
                f"❌ Advanced validation failed: {invalid_count}/{total_count} claims failed validation"
            )
            sys.exit(1)
