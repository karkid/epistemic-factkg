...
from src.core.gnn.train import TrainConfig, Trainer
from src.core.gnn.types import NUM_STANCE
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--dataset",        default=None,
                    help="Unused — kept for Justfile compatibility")
...
def main() -> None:
    ...
    if not jsonl_path.exists():
        print(f"Error: {jsonl_path} not found. Run 'just filter' first.", file=sys.stderr)
        sys.exit(1)
    ...
    if not args.no_class_weights:
        ...
        counts = torch.bincount(all_stance_y, minlength=NUM_STANCE).float().clamp(min=1.0)
        stance_weights = counts.sum() / (NUM_STANCE * counts)
        if args.verbose:
            print(f"Stance class weights: {[round(w,3) for w in stance_weights.tolist()]}")
    ...
    print(f"Training EpistemicHGNN V1  |  "
