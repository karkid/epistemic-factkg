...
from src.adapters.ai2thor.claims.types import ClaimCorpus
...
class ClaimGenerationStats:
    ...
    def finalize_timing(self) -> None:
        ...
        if self.total_processed_triples > 0 and self.processing_time_seconds > 0:
            self.average_time_per_triple = (
                self.processing_time_seconds / self.total_processed_triples
            )
    ...
    def corruption_success_rate(self) -> float:
        ...
        return (
            (self.successful_corruptions / total_attempts * 100)
            if total_attempts > 0
            else 0.0
        )
    ...
    def claim_generation_rate(self) -> float:
        ...
        return (
            (self.total_claims_generated / self.total_processed_triples * 100)
            if self.total_processed_triples > 0
            else 0.0
        )
...
class ClaimGenerationStatsSummary:
    ...
    def print_summary(self) -> None:
        ...
        if self.stats.refuted > 0:
            print(
                f"   Supported:Refuted ratio: {self.stats.supported_to_refuted_ratio:.2f}:1"
            )
        else:
            print(
                f"   Supported:Refuted ratio: {self.stats.supported}:0 (no refuted claims)"
            )

        if self.stats.duplicate_claims_filtered > 0:
            print(
                f"   Duplicates filtered:     {self.stats.duplicate_claims_filtered:,}"
            )

        # Corruption strategy breakdown
        if self.stats.corruption_by_type:
            print("\n🎯 CORRUPTION STRATEGIES:")
            for corruption_type, count in sorted(
                self.stats.corruption_by_type.items(), key=lambda x: x[1], reverse=True
            ):
        ...
        if self.stats.object_types_processed:
            ...
            sorted_objects = sorted(
                self.stats.object_types_processed.items(),
                key=lambda x: x[1],
                reverse=True,
            )
            for obj_type, count in sorted_objects[:10]:  # Top 10
                percentage = (
                    count / sum(self.stats.object_types_processed.values())
                ) * 100
        ...
        if self.stats.receptacle_mappings_used:
            ...
            sorted_receptacles = sorted(
                self.stats.receptacle_mappings_used.items(),
                key=lambda x: x[1],
                reverse=True,
            )
            for receptacle, count in sorted_receptacles[:5]:  # Top 5
                percentage = (
                    count / sum(self.stats.receptacle_mappings_used.values())
                ) * 100
        ...
        if self.stats.processing_time_seconds > 0:
            print("\n⏱️  PERFORMANCE:")
            print(
                f"   Total processing time:   {self.stats.processing_time_seconds:.2f} seconds"
            )
            if self.stats.average_time_per_triple > 0:
                print(
                    f"   Average time per triple: {self.stats.average_time_per_triple * 1000:.2f} ms"
                )
            print(
                f"   Processing rate:         {self.stats.total_processed_triples / self.stats.processing_time_seconds:.1f} triples/sec"
            )
