"""Cached data-loading functions for app.
...
def load_graph_cache(model_key: str, graph_cache_dir: Path) -> dict | None:
    # Split caches are written to out/model/graphs/ by the training pipeline
    p = graph_cache_dir / "graphs" / f"split_cache_{model_key}.pkl"
    if not p.exists():
        return None
    with open(p, "rb") as f:
...
def _resolve_run_dir(base: Path) -> Path:
    """Return the active run directory under base.

    Tries base/latest (symlink or directory) first, then base/latest.txt
    (Windows fallback), then base itself (legacy flat layout).
    """
    latest = base / "latest"
    if latest.exists():
        return latest
    txt = base / "latest.txt"
    if txt.exists():
        run_id = txt.read_text(encoding="utf-8").strip()
...
def load_verdict_metrics(model_key: str, reports_root: Path) -> dict | None:
    run_dir = _resolve_run_dir(reports_root / model_key / "eval")
    p = run_dir / "verdict_metrics.json"
    try:
...
def load_training_history(model_key: str, reports_root: Path) -> dict | list | None:
    run_dir = _resolve_run_dir(reports_root / model_key)
    p = run_dir / "training_history.json"
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except (FileNotFoundError, json.JSONDecodeError):
        return None
...
def get_predictor(model_key: str, models_root: Path, registry_path: Path | None = None):
    """Load and cache an EpistemicPredictor for the given model key."""
    from app.core.predictor import EpistemicPredictor
    try:
        return EpistemicPredictor(model_key, models_root=models_root, registry_path=registry_path)
