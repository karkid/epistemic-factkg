...
def render(cfg: "AppConfig") -> None:
    from app.core.loaders import load_all_records_list
    from app.core.state import load_record_into_state
    from app.core.widgets import render_claim_filter_bar, apply_claim_filter
    ...
    with col_btn:
        run = st.button("Search", type="primary", key="rec_run", width='stretch')
    with col_info:
    ...
    for rec in filtered[:filt.max_results]:
        ...
        with st.container(border=True):
            ...
            f1.caption(ev_info)
            if f2.button("Load in Verify", key=f"rec_load_{claim_id}", width='stretch'):
